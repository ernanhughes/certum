...
class EnergyResult:
    ...
    explained: float                 # ||U_r^T c||^2 in [0,1]
    topk: int
    rank_r: int
    sv: Optional[List[float]]        # singular values (for diagnostics)
    topk_scores: Optional[List[float]]
    used_count: int                  # number of evidence sentences actually used
...
def build_evidence_basis(
    ...
    *,
    ...
    """
    if E.size == 0:
        return np.zeros((c.shape[0], 0), dtype=np.float32), np.array([], dtype=np.float32), np.array([], dtype=np.int64)
...
def hallucination_energy_svd(
    ...
    *,
    ...
    if claim_vec is None or evidence_vecs is None:
        return EnergyResult(energy=1.0, explained=0.0, topk=top_k, rank_r=rank_r,
                            sv=None, topk_scores=None, used_count=0)
    ...
    explained = float(np.dot(proj_coords, proj_coords))  # scalar in [0,1] (approx)
    ...
    if not return_debug:
        return EnergyResult(energy=energy, explained=explained, topk=top_k, rank_r=rank_r,
                            sv=None, topk_scores=None, used_count=int(len(idx)))
